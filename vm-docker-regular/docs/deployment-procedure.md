...
AlmaLinux, or Oracle Linux 10+. Build one with `../pve-templates/` if you
don't have one yet.
...
5. **Ansible** (`ansible/`) connects as `fabricator` and runs
   ...
   - `base` — enables EPEL (distro-specific), upgrades all packages,
     ...
     enabled and started). This copy of the role has no `zram-generator`
     task at all — Docker hosts are the one case that shouldn't get it —
     so the task is simply absent here rather than conditional,
...
GitLab repo (copy the whole `vm-docker-regular/` folder out — its
...
`base`/`docker-ce`/`first-boot` — typically a docker-compose stack or a
container's systemd unit) and list it in `ansible/group_vars/all.yml`'s
`service_roles`:
...
after `base`, `docker-ce`, and `first-boot` (so Docker is already
