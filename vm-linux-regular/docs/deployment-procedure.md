...
Linux 10+ — the `base` role uses `dnf`.
...
`vm-linux-regular/` change, or on demand by starting a pipeline with the
variable `VM_TARGET=vm-linux-regular`. `apply`, `configure`, and `destroy`
