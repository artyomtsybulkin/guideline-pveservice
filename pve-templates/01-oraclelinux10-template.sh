#!/usr/bin/env bash
# Builds the Oracle Linux 10 template — hardware only, plus attaching the
...
# e.g. "local:iso/oraclelinux10.iso". The Proxmox host is not assumed to
...
VMID="${VMID:-9001}"
VM_NAME="${VM_NAME:-vm-oracle10}"
DISK_SIZE_GB="${DISK_SIZE_GB:-20}"
OS_ISO="${OS_ISO:-local:iso/oraclelinux10.iso}"
...
VM $VMID is ready for a manual install. This part is not scriptable:
  ...
  2. Run through the Anaconda installer as usual. XFS is Anaconda's
     default root filesystem on Oracle Linux 10 — no need to change it.
